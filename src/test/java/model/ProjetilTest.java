...
class ProjetilTest {
    ...
    void testInteligenciaArtificialDeProjetilQueSegue() {
        ...
        // Míssil nasce no ponto (0,0) olhando APENAS pra cima (velocidade Y), sem noção
        // do player
        ...
        // Injeta a IA de seguir
        missil.addComportamento(new ProjetilQueSegue());

        // Roda 1 frame de IA (60fps)
        missil.executar(0.16f);

        // Após a IA processar, o míssil deve ter dobrado pro eixo X em direção ao
        // player
        ...
        System.out.println(
                "Míssil rastreador passou no teste! Ele detectou o player a distância e corrigiu sua rota em direção a ele.");
    }

    @Test
    void testDesengatilhoAposDano() {
        // Player com conhecimento 50 garante shield > 0 (0.22*50 - 1.8 = 9.2 → shield =
        // 9)
        PlayerProva alvo = new PlayerProva(
        ...
        // Projétil nasce exatamente em cima do player (colisão garantida no primeiro
        // frame)
        ...
        System.out.println("Projétil colidiu, aplicou " + (shieldAntes - alvo.getShieldAtual())
                + " de dano no shield e se destruiu corretamente.");
    }
}
