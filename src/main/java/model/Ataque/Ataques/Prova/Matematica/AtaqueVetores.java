    ...
    protected void logicaAtaque(float dt) {
        ...
        if (timer >= (10 /(dificuldade/2)) && !this.isFinalizado() && projeteisLancados < limiteProjeteis) {
            ...
            float anguloParaPlayer = (float) Math.atan2(
                    (target.getHitbox().getCentro().getY() + target.getVelocidade().getY() * dt * framesNoFuturo) - posY,
                    (target.getHitbox().getCentro().getX() + target.getVelocidade().getX() * dt * framesNoFuturo) - posX
            );
